...
use modql::filter::{
    FilterGroups, FilterNode, FilterNodeOptions, ListOptions, OpVal, OpValBool, OpValFloat64,
    OpValInt32, OpValInt64, OpValString, OpValValue, OrderBy, OrderBys,
};
...
impl TryFrom<ProtoOpValJsonValue> for OpValValue {
    ...
    fn try_from(p: ProtoOpValJsonValue) -> Result<Self, Self::Error> {
        let parse = |s: String| {
            serde_json::from_str::<serde_json::Value>(&s).map_err(ProtoConversionError::InvalidJsonValue)
        };
